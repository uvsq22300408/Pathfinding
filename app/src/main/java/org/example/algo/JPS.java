...
public class JPS extends AStarAlgorithm {
  ...
  private Map<String, Integer> jumpPointCache = new LinkedHashMap<String, Integer>(1000, 0.75f, true) {
    @Override
    ...
    if (dx == 0 && dy == 0) {
      ...
      if (dx != 0 && dy != 0) {
        if (!isObstacle(x + dx, y))
          checkDirection(sommet, dx, 0, indexVoisins);
        if (!isObstacle(x, y + dy))
          checkDirection(sommet, 0, dy, indexVoisins);
      }

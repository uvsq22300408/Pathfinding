...
public class DrawJPS {
    public static void drawJPS(World world) {
        ...
        while(!WindowShouldClose()) {
            ...
            // Draw Path
            List<Point> path = JPSGrid.cheminFinal;
            for (int pointX = 0; pointX < path.size() - 1; pointX++) {
                Point a = path.get(pointX);
                Point b = path.get(pointX + 1);
                DrawLine(a.x * world.tailleReg, a.y * world.tailleReg,
                     b.x * world.tailleReg, b.y * world.tailleReg, RED);
            }
            EndMode2D();
            EndDrawing();
        }
